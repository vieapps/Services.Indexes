...
namespace net.vieapps.Services.Indexes
{
    public class ServiceComponent : ServiceBase
    {
        ...
        async Task<JToken> ProcessStockIndexesAsync(RequestInfo requestInfo, CancellationToken cancellationToken)
        {
            ...
            JArray.Parse(await new Uri("http://banggia.cafef.vn/stockhandler.ashx?index=true").FetchHttpAsync(cancellationToken).ConfigureAwait(false))
                .Select(stockIndex => stockIndex as JObject)
                .ToList().ForEach(stockIndex =>
                {
        ...
        async Task<JToken> ProcessStockQuoteAsync(RequestInfo requestInfo, CancellationToken cancellationToken)
        {
            ...
            try
            {
                using (var webResponse = await new Uri("https://finance.vietstock.vn").SendHttpRequestAsync(headers, 90, cancellationToken).ConfigureAwait(false))
                {
                    var cookies = webResponse.Cookies.ToList();
                    ...
                    {
                        ["Cookie"] = $"language=vi-VN,finance_viewedstock={code}{(string.IsNullOrWhiteSpace(sessionID) ? "" : $",ASP.NET_SessionId={sessionID}")}{(string.IsNullOrWhiteSpace(cookieTokenID) ? "" : $",__RequestVerificationToken={cookieTokenID}")}",
                        ["Content-Type"] = "application/x-www-form-urlencoded; charset=utf-8",
                        ["X-Requested-With"] = "XMLHttpRequest"
                    };

                    var html = await webResponse.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
                    ...
                    using (var restResponse = await new Uri("https://finance.vietstock.vn/company/tradinginfo").SendHttpRequestAsync("POST", headers, body, 90, cancellationToken).ConfigureAwait(false))
                    {
                        var results = await restResponse.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
            ...
            try
            {
                using (var webResponse = await new Uri($"https://finance.vietstock.vn/search/{code.UrlEncode()}").SendHttpRequestAsync(headers.Where(kvp => !kvp.Key.IsEquals("X-Requested-With") && !kvp.Key.IsEquals("Content-Type")).ToDictionary(kvp => kvp.Key, kvp => kvp.Value), 90, cancellationToken).ConfigureAwait(false))
                {
                    var companyInfo = await webResponse.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
